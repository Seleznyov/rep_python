...
def get_cur_by_abbr(currencies,abbr):
	for kurs in currencies:
		if kurs.get("Cur_Abbreviation") == abbr:
			return kurs
print(get_cur_by_abbr(currencies, curr))

def create_message_about_curr(cur):
	return (f'Курс {cur.get("Cur_Abbreviation")} на сегодня: {cur.get("Cur_OfficialRate")} рубля за {cur.get("Cur_Scale")} {cur.get("Cur_Abbreviation")}.')
print(create_message_about_curr(get_cur_by_abbr(currencies, curr)))
